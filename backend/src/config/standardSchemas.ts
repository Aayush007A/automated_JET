...
export const SPARK_TB_FIELDS: StandardFieldDefinition[] = [
  { name: 'G/L', label: 'G/L', type: 'string', required: true, aliases: ['g/l', 'g_l', 'gl', 'account', 'gl account', 'account number', 'gl code', 'account_number', 'saknr', 'hkont'], description: 'G/L Account Code' },
  { name: 'Description', label: 'Description', type: 'string', required: true, aliases: ['description', 'gl description', 'account desc', 'account name', 'account_description', 'txt20', 'txt50'], description: 'Account Description' },
  { name: 'Account Subtype', label: 'Account Subtype', type: 'string', required: true, aliases: ['account subtype', 'account_subtype', 'subtype', 'acct subtype', 'account type'], description: 'Assets, Liabilities, Revenue/Income, Expenses, Equity' },
  { name: 'Opening Balance', label: 'Opening Balance', type: 'number', required: true, aliases: ['opening balance', 'opening_balance', 'op balance', 'open bal', 'beginning balance', 'beginning_balance_ec'], description: 'Opening balance for the period' },
  { name: 'Debit', label: 'Debit', type: 'number', required: false, aliases: ['debit', 'debit amount', 'total debit', 'dr', 'debit_amount'], description: 'Total debit activity' },
  { name: 'Credit', label: 'Credit', type: 'number', required: false, aliases: ['credit', 'credit amount', 'total credit', 'cr', 'credit_amount'], description: 'Total credit activity' },
  { name: 'Closing Balance', label: 'Closing Balance', type: 'number', required: true, aliases: ['closing balance', 'closing_balance', 'end balance', 'balance', 'close bal', 'ending balance', 'ending_balance_ec'], description: 'Closing balance for the period' },
  { name: 'FS Line Item', label: 'FS Line Item', type: 'string', required: false, aliases: ['fs line item', 'fs_line_item', 'fs line', 'financial statement line', 'financial_statement_line'], description: 'Financial statement line item' }
];

export const SPARK_GL_FIELDS: StandardFieldDefinition[] = [
  { name: 'G/L', label: 'G/L', type: 'string', required: true, aliases: ['g/l', 'gl account', 'g_l', 'gl', 'account', 'account number', 'gl code', 'account_number', 'saknr', 'hkont'], description: 'G/L Account Number' },
  { name: 'DocumentNo', label: 'DocumentNo', type: 'string', required: true, aliases: ['documentno', 'accounting document', 'document number', 'doc no', 'journal_number', 'belnr', 'voucher no', 'je number', 'doc_num'], description: 'Accounting Document Number' },
  { name: 'Type', label: 'Type', type: 'string', required: true, aliases: ['type', 'document type', 'transaction_type', 'doc type', 'blart', 'trans type'], description: 'Document / Transaction Type (e.g. SA, AB, RV)' },
  { name: 'Entry Date', label: 'Entry Date', type: 'date', required: true, aliases: ['entry date', 'entry_date', 'accounting date', 'date_effective', 'effective date', 'doc_date', 'bldat'], description: 'Entry Date' },
  { name: 'Pstng Date', label: 'Pstng Date', type: 'date', required: true, aliases: ['pstng date', 'pstng_date', 'posting date', 'date_posted', 'budat', 'posted date', 'post date'], description: 'Posting Date' },
  { name: 'Doc. Date', label: 'Doc. Date', type: 'date', required: false, aliases: ['doc. date', 'doc date', 'document date', 'document_date', 'bldat'], description: 'Document Date' },
  { name: 'Amount in local cur.', label: 'Amount in local cur.', type: 'number', required: true, aliases: ['amount in local cur.', 'amount in local cur', 'amount in local currency', 'amount_in_local_cur', 'net_amount_ec', 'amount', 'dmbtr', 'net amount'], description: 'Amount in Local Currency' },
  { name: 'LCurr', label: 'LCurr', type: 'string', required: true, aliases: ['lcurr', 'currency code', 'currency', 'local currency', 'curr', 'entity_currency_ec', 'waers'], description: 'Local Currency Code' },
  { name: 'Amount in doc. curr.', label: 'Amount in doc. curr.', type: 'number', required: false, aliases: ['amount in doc. curr.', 'amount in doc curr', 'wrbtr', 'amount in doc currency'], description: 'Amount in Document Currency' },
  { name: 'Curr.', label: 'Curr.', type: 'string', required: false, aliases: ['curr.', 'curr', 'doc curr', 'document currency', 'waers'], description: 'Document Currency Code' },
  { name: 'Amount in loc.curr.2', label: 'Amount in loc.curr.2', type: 'number', required: false, aliases: ['amount in loc.curr.2', 'amount in loc curr 2', 'dmbe2', 'net_amount_gc'], description: 'Amount in Local Currency 2' },
  { name: 'LCur2', label: 'LCur2', type: 'string', required: false, aliases: ['lcur2', 'group currency', 'curr2', 'group_currency_gc', 'hwaer'], description: 'Local Currency 2 Code' },
  { name: 'Document Header Text', label: 'Document Header Text', type: 'string', required: false, aliases: ['document header text', 'text header', 'header text', 'journal_header_description', 'bktxt', 'header description'], description: 'Document Header Text' },
  { name: 'Text', label: 'Text', type: 'string', required: false, aliases: ['text', 'text details', 'item text', 'line text', 'journal_line_description', 'sgtxt', 'narrative'], description: 'Line Item Text' },
  { name: 'User name', label: 'User name', type: 'string', required: true, aliases: ['user name', 'user_name', 'username', 'user ud', 'userid_entered', 'user id', 'entered by', 'usnam', 'created by', 'user'], description: 'User Name / User ID' }
];
...
export const OMNIA_TB_FIELDS: StandardFieldDefinition[] = [
  ...
  { name: 'beginning_balance_ec', label: 'Beginning Balance (EC)', type: 'number', required: true, aliases: ['opening balance', 'opening_balance', 'op balance', 'open bal', 'beginning balance'], description: 'Opening balance in entity currency' },
  ...
  { name: 'debit', label: 'Debit', type: 'number', required: false, aliases: ['debit amount', 'total debit', 'dr', 'debit_amount'], description: 'Total debit amount' },
  { name: 'credit', label: 'Credit', type: 'number', required: false, aliases: ['credit amount', 'total credit', 'cr', 'credit_amount'], description: 'Total credit amount' },
  ...
  { name: 'group_currency_gc', label: 'Group Currency (GC)', type: 'string', required: false, aliases: ['reporting currency', 'group curr', 'parent currency'], description: 'Presentation group currency code' },
  ...
  { name: 'ending_balance_gc', label: 'Ending Balance (GC)', type: 'number', required: false, aliases: ['closing balance gc', 'end bal gc'], description: 'Ending balance in group currency' },
  { name: 'financial_statement_line', label: 'FS Line Item', type: 'string', required: false, aliases: ['fs_line_item', 'fs line', 'financial statement line', 'bs/is line'], description: 'Financial statement line item' },
  { name: 'chart_of_accounts', label: 'Chart of Accounts', type: 'string', required: false, aliases: ['coa', 'chart of account', 'ktopl'], description: 'Chart of accounts identifier' }
];
